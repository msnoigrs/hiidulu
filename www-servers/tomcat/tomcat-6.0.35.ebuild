...
IUSE="examples test dbcp"

COMMON_DEPEND="dev-java/eclipse-ecj:3.7
	dev-java/ant-eclipse-ecj:3.7
	>=dev-java/commons-daemon-1.0.1
	dev-java/jcl-over-slf4j
	~dev-java/tomcat-servlet-api-${PV}
	dbcp? ( dev-java/tomcat-dbcp:7
...
java_prepare() {
	epatch "${FILESDIR}/${SLOT}/build-xml-6.0.33.patch"
...
src_compile(){
	...
	if use amd64 && [[ "${GENTOO_VM}" = "sun-jdk-1.5" ]] ; then
			java-pkg_force-compiler ecj-3.7
	fi

#	local antflags="build-jasper-jdt deploy -Dbase.path=${T}"
	local antflags="deploy -Dbase.path=${T}"
	...
	antflags="${antflags} -Djdt.jar=$(java-pkg_getjar eclipse-ecj-3.7 ecj.jar)"
...
#		|| die "failed to copy"
	...
	if use dbcp; then
		java-pkg_jar-from tomcat-dbcp-7
	...
	if use doc; then
#		 cp -pr webapps/docs ${WEBAPPS_DIR}
		cp -pr webapps/docs "${D}"${TOMCAT_HOME}/webapps/ROOT
	fi
	if use examples; then
		...
		cd "${S}"
		cp -pPr webapps/examples ${WEBAPPS_DIR}
	fi
...
pkg_postinst() {
	...
	if [[ -d "/var/lib/${TOMCAT_NAME}/webapps/ROOT" ]] ; then
		...
		cp -pr "${ROOT}"/usr/share/${TOMCAT_NAME}/webapps/ROOT \
			"${ROOT}""${WEBAPPS_DIR}" || die
#		cp -pr "${ROOT}"/usr/share/${TOMCAT_NAME}/webapps/docs \
#			"${ROOT}""${WEBAPPS_DIR}" || die
	fi
	...
	elog "Execute the following command to setup ${P} to work with 'dev-util/netbeans:6.7'"
	elog
...
pkg_config() {
	...
	einfo "This Configure will setup your ${P} to be used within dev-util/netbeans:6.7"
	...
	einfo
	# check for Netbeans
	if ! has_version dev-util/netbeans:6.7 ; then
		eerror "Can't configure Tomcat for Netbeans without Netbeans!"
		eerror "Please 'emerge dev-util/netbeans:6.7"
	fi

	# make sure all Java EE NB Modules are installed to prevent
	# ClassDefNotFoundError
	if ! built_with_use dev-util/netbeans:6.7 netbeans_modules_enterprise ; then
	   eerror "Please rebuild 'dev-util/netbeans:6.7 with NETBEAN_MODULES='enterprise
	   mobility groovy'"
	   die "Netbeans lacks required Modules for EE deployment"
	fi

	#add netbeans run user to tomcat group to prevent permission issues
	einfo "Please enter the user that runs 'dev-util/netbeans:6.7': "
	read user
	...
	einfo
	einfo "You are now ready to start 'dev-util/netbeans:6.7' and add "
