...
COMMON_DEPEND="dev-java/eclipse-ecj:3.5
	dev-java/ant-eclipse-ecj:3.5
...
src_unpack() {
	...
	epatch "${FILESDIR}/${SLOT}/build-xml-6.0.20.patch"
...
src_compile(){
	...
	if use amd64 && [[ "${GENTOO_VM}" = "sun-jdk-1.5" ]] ; then
	        java-pkg_force-compiler ecj-3.5
	fi
	...
	antflags="${antflags} -Djdt.jar=$(java-pkg_getjar eclipse-ecj-3.5 ecj.jar)"
...
#		|| die "failed to replace catalina.policy"
	...
	if use dbcp; then
		java-pkg_jar-from tomcat-dbcp-${SLOT}
