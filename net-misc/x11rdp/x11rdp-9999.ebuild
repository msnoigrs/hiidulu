...
SRC_URI="http://xorg.freedesktop.org/releases/individual/xserver/${XORG_P}.tar.bz2"
...
src_prepare() {
	...
	# -- xrdp patches --
	epatch "${FILESDIR}"/xrdp-0.8.0-0001-Include-xorg-list.h-to-fix-build-errors.patch
	epatch "${FILESDIR}"/xrdp-0.8.0-0002-Remove-dither-printing.patch
	...
	local xorg_patches=(
		...
		"${FILESDIR}"/gcc5-patch.patch
		"${FILESDIR}"/gcc5-patch2.patch
	)
	cd "${XORG_P}" || die
	epatch "${xorg_patches[@]}"
