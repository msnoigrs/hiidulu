...
#inherit unpacker eutils python-single-r1 systemd
inherit unpacker eutils python-single-r1

AT_AVAILABLE=( amd64 x86 )
AT_amd64="${PN}_current_amd64.deb"
AT_x86="${PN}_current_i386.deb"
...
#BASE_URI="https://dl.google.com/linux/chrome-remote-desktop/deb/pool/main/c/${PN}/"
BASE_URI="http://dl.google.com/linux/direct/"
SRC_URI="amd64? ( ${AT_amd64} )
	x86? ( ${AT_x86} )"
...
pkg_nofetch() {
	...
	check_tarballs_available "${BASE_URI}" "${distfiles[@]}"
}
...
src_install() {
	...
	dosym /etc/opt/chrome/native-messaging-hosts /etc/chromium/
}
