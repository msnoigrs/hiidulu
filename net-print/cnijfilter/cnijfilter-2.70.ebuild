...
src_unpack() {
	...
	epatch ${FILESDIR}/cnijfilter-2.70-pstocanonij-c.patch || die
	epatch ${FILESDIR}/gtk-2.patch || die
}

src_compile() {
	cd libs || die
	./autogen.sh --prefix=${_prefix} || die "Error: libs/autoconf.sh failed"
	emake CC="$(tc-getCC)" CFLAGS="${CFLAGS}" || die "Couldn't make libs"

	cd ../pstocanonij || die
	./autogen.sh --prefix=/usr --enable-progpath=${_bindir} || die "Error: pstocanonij/autoconf.sh failed"
	emake CC="$(tc-getCC)" CFLAGS="${CFLAGS}" || die "Couldn't make pstocanonij"

	if use servicetools; then
		cd ../cngpij || die
		./autogen.sh --prefix=${_prefix} --enable-progpath=${_bindir} || die "Error: cngpij/autoconf.sh failed"
		emake CC="$(tc-getCC)" CFLAGS="${CFLAGS}" || die "Couldn't make cngpij"

		cd ../cngpijmon || die
		./autogen.sh --prefix=${_prefix} || die "Error: cngpijmon/autoconf.sh failed"
		emake CC="$(tc-getCC)" CFLAGS="${CFLAGS} -DGTK_ENABLE_BROKEN" || die "Couldn't make cngpijmon"
	fi
