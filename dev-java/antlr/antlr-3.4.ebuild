...
COMMON_DEPEND=">=dev-java/stringtemplate-4.0.4:4
...
antlr33() {
	java -cp $(java-pkg_getjars --build-only antlr-3.3,antlr,stringtemplate) org.antlr.Tool "${@}" || die "antlr3 failed"
}

antlr3() {
	local cp="${S}/bootstrap:${S}/tool/src/main/resources/"
	java -cp "${cp}":$(java-pkg_getjars stringtemplate-4) \
...
build_antlr() {
	...
	ejavac -d "${dest}" -cp $(java-pkg_getjars stringtemplate,stringtemplate-4) "@${T}/sources"
}
...
src_install() {
	...
	java-pkg_dolauncher antlr3.4 --main org.antlr.Tool
