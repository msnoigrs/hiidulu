...
QA_PRESTRIPPED="usr/share/{PN}-{SLOT}/bin/nativeexecutoion/*/pty"
...
src_prepare() {
	...
	java-pkg_jar-from --into html.parser/external icu4j-4.6 icu4j.jar icu4j-4_0.jar
	java-pkg_jar-from --into html.validation/external jcl-over-slf4j jcl-over-slf4j.jar commons-logging-1.1.1.jar
	java-pkg_jar-from --into html.validation/external jcl-over-slf4j jcl-over-slf4j.jar commons-logging-api-1.1.1.jar
	...
	java-pkg_jar-from --into libs.ini4j/external ini4j-0.5 ini4j.jar ini4j-0.5.1.jar
	java-pkg_jar-from --into libs.jaxb/external jaxb-2.2 jaxb-impl.jar jaxb-impl.jar
	...
	jsr173_ln=$(egrep -n jsr173_api.jar xml.jaxb.api/nbproject/project.xml | sed 's/^\([1-9][0-9]*\):.*$/\1/')
...
src_install() {
	...
	# bytelist-0.1.jar
	rm commons-logging-api-1.1.1.jar && dosym /usr/share/jcl-over-slf4j/lib/jcl-over-slf4j.jar ${instdir}/commons-logging-api-1.1.1.jar || die
	rm commons-logging-1.1.1.jar && dosym /usr/share/jcl-over-slf4j/lib/jcl-over-slf4j.jar ${instdir}/commons-logging-1.1.1.jar || die
	...
	rm icu4j-4_0.jar && dosym /usr/share/icu4j-4.6/lib/icu4j.jar ${instdir}/icu4j-4_0.jar || die
