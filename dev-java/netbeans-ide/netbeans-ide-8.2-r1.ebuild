...
src_prepare() {
	...
	jaf_ln=$(egrep -n activation.jar xml.jaxb.api/nbproject/project.xml | sed -e 's/^\([1-9][0-9]*\):.*$/\1/')
...
src_compile() {
	unset DISPLAY
	eant -f ${EANT_BUILD_XML} ${EANT_EXTRA_ARGS} ${EANT_BUILD_TARGET} -Dpermit.jdk8.builds=true || die "Compilation failed"
}
...
src_install() {
	...
	rm ini4j-0.5.1.jar && java-pkg_jar-from --into "${instdir}" ini4j-0.5 ini4j.jar ini4j-0.5.1.jar
	...
	rm log4j-1.2.15.jar && java-pkg_jar-from --into "${instdir}" log4j-over-slf4j log4j-over-slf4j.jar log4j-1.2.15.jar
	...
	rm resolver-1.2.jar && java-pkg_jar-from --into "${instdir}" xml-commons-resolver xml-commons-resolver.jar resolver-1.2.jar
	rm saxon9B.jar && java-pkg_jar-from --into "${instdir}" saxon-9.3 saxon9.jar saxon9B.jar
	rm servlet-2.2.jar && java-pkg_jar-from --into "${instdir}" tomcat-servlet-api-2.2 servlet.jar servlet-2.2.jar
	rm smack.jar && java-pkg_jar-from --into "${instdir}" smack-3 smack.jar
	rm smackx.jar && java-pkg_jar-from --into "${instdir}" smack-3 smackx.jar
	...
	pushd "${instdir}" >/dev/null || die
	rm jaxb-api.jar && java-pkg_jar-from --into "${instdir}" jaxb-2.2 jaxb-api.jar jaxb-api.jar
	popd >/dev/null || die
