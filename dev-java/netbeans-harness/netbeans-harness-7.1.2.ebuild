...
src_prepare() {
	...
	java-pkg_jar-from --into jemmy/external jemmy-2 jemmy.jar jemmy-2.3.0.0.jar
...
src_install() {
	...
	pushd "${D}"/${instdir} >/dev/null || die
	# jemmy-2.3.0.0.jar
	rm jemmy-2.3.0.0.jar && dosym /usr/share/jemmy-2/lib/jemmy.jar ${instdir}/jemmy-2.3.0.0.jar
	popd >/dev/null || die
