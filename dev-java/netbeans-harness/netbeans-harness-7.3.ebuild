...
src_prepare() {
	...
	java-pkg_jar-from --into jemmy/external jemmy-2 jemmy.jar jemmy-2.3.1.1.jar
...
src_install() {
	...
	pushd "${D}"/${instdir} >/dev/null || die
	rm jemmy-2.3.1.1.jar && dosym /usr/share/jemmy-2/lib/jemmy.jar ${instdir}/jemmy-2.3.1.1.jar || die
	popd >/dev/null || die
