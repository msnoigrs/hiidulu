...
java_prepare() {
	...
	pushd ../${ICEDTEA_PKG} > /dev/null || die
		epatch "${FILESDIR}"/endorsed1.patch
		#epatch "${FILESDIR}"/endorsed2.patch
		epatch "${FILESDIR}"/endorsed3.patch
		epatch "${FILESDIR}"/endorsed4.patch
	...
	mkdir "${WORKDIR}/endorsed"
	cd "${WORKDIR}/endorsed"
	java-pkg_jar-from --build-only xalan xalan.jar
	java-pkg_jar-from --build-only xalan serializer.jar
	java-pkg_jar-from --build-only xml-commons-external-1.4 xml-apis.jar
...
src_configure() {
	...
	ECONF_SOURCE="../${ICEDTEA_PKG}" econf ${config} \
		--with-endorsed-dir="${WORKDIR}/endorsed" \
