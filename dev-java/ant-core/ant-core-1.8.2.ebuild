...
SRC_URI="mirror://apache/ant/source/${MY_P}-src.tar.bz2
	https://github.com/masayuko/hiidulu/raw/master/distfiles/ant-${PV}-build.xml"
LICENSE="Apache-2.0"
SLOT="0"
...
src_prepare() {
	...
	cp "${FILESDIR}/ant" ${WORKDIR}
	eprefixify "${WORKDIR}/ant"

	# use our split-ant build.xml
	#mv -f "${FILESDIR}/${PV}-build.xml" . || die
	#cp -f "${FILESDIR}/ant-${PV}-build.xml" build.xml || die
	cp -f "${DISTDIR}/ant-${PV}-build.xml" build.xml || die
