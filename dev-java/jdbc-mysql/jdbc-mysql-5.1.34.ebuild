...
java_prepare() {
	epatch "${FILESDIR}/5.1.34-remove-hibernate.patch"

	epatch "${FILESDIR}/5.1.34-remove-jboss-dependency-from-tests.patch"
	# http://bugs.mysql.com/bug.php?id=28286
	epatch "${FILESDIR}/5.1.34-dist-target-depends.patch"
	# use java6 for everything except jdbc3 - #283848
	epatch "${FILESDIR}/5.1.34-java6.patch"

	echo "com.mysql.jdbc.jdk5.javac=$(java-config -c)" > "${S}/build.properties"
	echo "com.mysql.jdbc.jdk6=$(java-config -O)" >> "${S}/build.properties"

	rm -v *.jar || die

	# use test && mkdir src/lib-nodist # needed, or ant will fail

	rm -rv src/com/mysql/fabric/hibernate

	cd src/lib
	rm -v *.jar || die
	java-pkg_jar-from slf4j-api slf4j-api.jar
#	java-pkg_jar-from jcl-over-slf4j jcl-over-slf4j.jar commons-logging.jar
#	use log4j && java-pkg_jar-from log4j-over-slf4j log4j-over-slf4j.jar log4j.jar
