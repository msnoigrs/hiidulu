...
MyPV=${PV%b}
SRC_URI="mirror://sourceforge/saxon/Saxon-B/${PV}/saxonb${MyPV//./-}source.zip"
...
#	sed -i -e '/com.saxonica.SchemaAwareTransformerFactory/d' net/sf/saxon/ant/AntTransform.java
	...
	cp -i "${FILESDIR}/saxon-build.xml" build.xml || die
...
src_install() {
	...
	java-pkg_dolauncher ${PN}9 --main net.sf.saxon.Transform
	if use doc; then
