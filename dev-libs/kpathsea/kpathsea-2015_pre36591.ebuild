...
SRC_URI="http://dev.gentoo.gr.jp/~igarashi/distfiles/texlive-${PV}-source.tar.xz
...
S=${WORKDIR}/texlive-${PV}-source/texk/${PN}
...
src_prepare() {
	cd "${WORKDIR}/texlive-${PV}-source"
	S="${WORKDIR}/texlive-${PV}-source" elibtoolize #sane .so versionning on gfbsd
